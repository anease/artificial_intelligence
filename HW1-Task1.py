...
class State:
    def __init__(self, id, parentId, puzzleBoard, costToNode, costToGoal, solutionCost, priority):
        self.id = id
...
def findSucessorStates(parentState):
    ...
    for tileIndex in movableTileIndexes:
        ...
        if goalFlag == 0:
            ...
            childPriority = parentState.priority + 1

            finalState = State(nextStateId, parentState.id, sucessorBoard, totalCost, 0, 0, childPriority)
        ...
        if flag == 1 or not closedList:
            ...
            generateState(parentState.id, sucessorBoard, totalCost, childPriority)
...
def generateState(parentId, board, costToNode, priority):
    global nextStateId
    global openListCount

    newState = State(nextStateId, parentId, board, costToNode, 0, 0, priority)

    ## Iterate global variable for the next state
    nextStateId += 1

    openList.append((priority, newState))
    openListCount += 1
...
def initInputFile():
    ...
    generateState(0, startState, 0, 0)
...
def printBoard(state):
    ...
    print("Depth Level:", state.priority)
